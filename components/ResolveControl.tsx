...
export function ResolveControl({ market }: { market: MarketInfo }) {
  ...
  // read the creator (to show the withdraw-subsidy button only to them)
  ...
  const isCreator =
    !!address && !!creator && (address as string).toLowerCase() === (creator as string).toLowerCase();
  ...
  async function withdrawSubsidy() {
    if (!address) return;
    setBusy(true);
    try {
      await writeContractAsync({
      ...
      haptic("success");
      (window as any).Telegram?.WebApp?.showAlert?.("Subsidy withdrawn!");
    } catch (e: any) {
      haptic("error");
      (window as any).Telegram?.WebApp?.showAlert?.(
  ...
  if (market.state === MarketState.Resolved) {
    // only the creator sees the withdraw-subsidy button
    if (!isCreator) return null;
    return (
      <div className="glass mt-2 p-3">
        <div className="text-xs text-white/50">
          Market resolved. As the creator, you can withdraw your remaining
          subsidy (winners' payouts are kept aside automatically).
        </div>
        <button
          onClick={withdrawSubsidy}
          disabled={busy}
          className="mt-2 w-full rounded-xl py-2 text-sm font-semibold disabled:opacity-40"
          style={{ background: "#16c78422", color: "#16c784" }}
        >
          {busy ? "..." : "Withdraw subsidy"}
        </button>
      </div>
    );
  }
