...
export function DailyTab() {
  ...
  const { data: roundId } = useReadContract({
    ...
    query: { refetchInterval: 60000, refetchOnWindowFocus: false },
  });
  ...
  const { data: round, refetch: refetchRound } = useReadContract({
    ...
    query: { enabled: rid > 0, refetchInterval: 60000, refetchOnWindowFocus: false },
  });
  ...
  const { data: myVotes, refetch: refetchVotes } = useReadContract({
    ...
    query: { enabled: rid > 0 && !!address, refetchInterval: 45000, refetchOnWindowFocus: false },
  });
  ...
  const { data: winner } = useReadContract({
    ...
    args: rid && address ? [BigInt(rid), address] : undefined,
    query: { enabled: rid > 0 && !!address, refetchOnWindowFocus: false },
  });
