...
import { useDesktopShell } from "@/lib/useIsTelegram";
...
export default function Home() {
  ...
  // null until we know whether we're in Telegram or a browser
  ...
  const deployWarning = notDeployed && (
  ...
  const marketsSection = (
    <section>
      {/* filters — plus the create button on the browser layout, where a
          floating pill over the content has nothing to anchor to */}
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        {desktop && !notDeployed && (
          <button
            ...
            className="ml-auto rounded-full px-5 py-2 text-xs font-bold text-white transition active:scale-95"
            style={{ background: "var(--usdc)", boxShadow: "0 6px 18px -6px rgba(43,108,246,0.6)" }}
          >
            + Create market
          </button>
        )}
  ...
  // the tab content, shared by both layouts
  const content = (
    <>
      {deployWarning}
  ...
  const overlays = (
    <>
      {selected && <TradeSheet market={selected} onClose={() => setSelected(null)} />}
      {creating && (
        <CreateMarketSheet
